...
결과는 탐지 엔진이 어느 시각을 보는가에 따라 갈렸다. 수집 시점을 이벤트 시각으로 쓰는 구성(M2)에서는 폐기되는 이벤트가 원리적으로 없는데도 경보가 최대 22% 줄고 탐지 주체가 흔하게 빠졌으며, 악성 주체 회피율은 최대 57%였다. 센서의 발생 시각을 보존하는 구성(M1)에서는 두 영역이 나뉘었다. 윈도우를 넘는 지연은 해당 이벤트를 삭제한 것과 같은 효과를 내면서(135셀 중 97%에서 차이 0.01 이내) 지연된 이벤트만큼의 폐기 기록을 남겼다. 윈도우 이하의 지연은 흔적 없이 경보를 최대 16% 줄였지만, 10,800회 실행에서 탐지 주체를 단 하나도 빼내지 못했다. 악성 주체를 빼낸 M1 실행은 예외 없이 폐기를 동반했으나, 표적 지연에서는 그 흔적이 중앙값 260건, 최소 8건에 불과해 실무 감시로 드러날지는 보장되지 않는다. 규칙별 취약도 순위는 두 구성에서 반대로 나타났다(ρ = −0.61). 공격자가 자신의 이벤트만 골라 지연시키는 전략은 두 구성 모두에서 악성 주체를 더 많이 빼내지 못했다(주 분석 315셀 중 표적 우위 0). 다만 M1에서는 같은 조건의 무작위 지연보다 폐기를 중앙값 기준 약 18분의 1만 남겼다.
...
1. **파이프라인 구성에 따른 영향의 분리.** 탐지 엔진이 수집 시점을 이벤트 시각으로 쓰는 구성(M2)과 센서의 발생 시각을 보존하는 구성(M1)을 나누어 측정했다. 흔적 없이 주체 단위 탐지를 무너뜨리는 위협은 M2에 있었다. M1에서는 주체를 빼내는 지연이 항상 폐기 기록을 남겼지만, 표적 지연에서는 그 기록이 수십 건 수준으로 작을 수 있었다.

2. **취약성이 규칙과 구성의 상호작용에서 온다는 관찰과 표적 지연의 이중성.** 규칙 다섯 종의 취약도 순위가 두 구성에서 반대로 나타남을 보인다. 또한 부트스트랩 동등성 판정으로, 공격자가 자기 이벤트만 지연시키는 전략이 악성 주체를 더 많이 빼내지는 못하지만 M1에서 남기는 흔적은 크게 줄인다는 것을 보인다.
...
**윈도우를 넘는 지연은 삭제와 같다 (H1 채택).** 2·5·10배 지연의 135셀 중 97.0%에서 M1의 경보 감소가 삭제 대조군과 0.01 이내로 같았다. 벗어난 4셀은 h051의 희소한 스트림(remote_thread, beacon)이었다. 워터마크가 느리게 전진해 지연된 이벤트 일부가 폐기를 피하면서 피해가 삭제보다 오히려 작았다. 윈도우를 넘는 지연의 효과는 그 이벤트를 지운 것과 같으며, **지연된 이벤트만큼의 폐기 기록을 남긴다.** 기록의 크기는 지연 규모에 비례한다. 20% 무작위 지연에서는 규칙에 따라 중앙값 383건(remote_thread)에서 78,750건(net_scan)이었지만, 1% 표적 지연에서는 중앙값 1~70건이었다.
...
1.0배로 다가가는 곡선은 연속이다. 칼날 효과가 아니라, 지연이 윈도우에 가까울수록 그 사이 상태 초기화가 일어났을 확률이 커지는 실제 현상이다. 그러나 흔적 없는 피해의 대부분은 윈도우의 90%를 넘는 좁은 구간에 있고, 윈도우를 0.1%만 넘어도 폐기가 대량으로 발생한다(이 조건의 net_scan에서 11,914건). 흔적 없이 최대 피해를 내려면 공격자가 지연을 정밀하게 맞춰야 한다.
...
- **M1에서 주체를 빼내는 지연은 항상 폐기 흔적을 남겼다.** 흔적 없는 지연은 경보를 최대 16% 줄였지만 주체를 하나도 빼내지 못했다. 다만 표적 지연이 남긴 흔적은 중앙값 260건으로 작았다.
...
세 가지 방어는 서로를 보완한다. **발생 시각 보존**은 M2의 흔적 없는 회피를 M1의 흔적 있는 회피로 바꾸고, **폐기 이벤트 감시**는 그 흔적을 찾을 기회를 주며, **재정렬 버퍼**는 흔적의 원인 자체를 줄인다. 다만 표적 지연의 흔적은 수십~수백 건으로 작을 수 있으므로(§V-2), 폐기 감시가 효과적이려면 정상 운영의 폐기 기저선을 알고 그보다 작은 변화를 잡을 수 있어야 한다. 이 조건은 본 연구가 확인하지 않았다. 반면 재정렬 버퍼는 흔적의 크기와 무관하게, 지연이 버퍼 안에 들어오면 판정을 되돌린다.
...
공격자의 정밀 제어 능력이 주는 이득은 지표에 따라 다르다. **회피율로는 이득이 없었다.** 표적 지연이 악성 주체를 더 많이 빼내지는 못했으므로(§V-5), 부하로 전반적인 지연만 유발해도 같은 수의 주체를 빼낼 수 있다. 그러나 **M1의 은밀성으로는 이득이 컸다.** 규칙·사례·비율·지연이 같은 셀끼리 비교하면, 표적 지연이 남긴 폐기는 무작위 지연의 중앙값 기준 약 18분의 1이었다(회피가 있었던 94셀, 셀에 따라 2분의 1~63분의 1). 회피율은 거의 같았다. 정리하면 M2에서는 공격자 요구 수준이 낮고, M1에서 흔적을 줄이려면 자신의 이벤트만 골라 지연시키는 능력이 필요하다. M1에서 흔적 없이 최대 경보 감소를 내려면 지연을 윈도우 바로 아래로 정밀하게 맞춰야 하지만, 그 경우 주체는 빠지지 않는다.
...
실행 시간은 측정 환경에서 E3 약 25분, E4 약 25분, E5 약 45분, E6 분포당 약 17분, E1 약 25분이다. 난수 시드가 고정되어 있어 같은 입력에서 같은 레코드가 같은 순서로 나오므로, 동봉본과는 파일 비교로 대조할 수 있다. 판정은 `python scripts/analyze_v2.py --results $OUT --output $OUT/analysis.json`으로 한다(기본 입력은 `data/p1/v2/`). 본문에서 "탐색적 분석"으로 표시한 수치(주체 손실의 영역별 분포, 흔적 크기, 윈도우 경계 부근의 촘촘한 지연)는 `python scripts/explore_v2.py --results $OUT --output $OUT/exploratory.json`으로 다시 계산한다.
